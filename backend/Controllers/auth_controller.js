...
const register = async (req, res) => {
  try {
    ...
    us.profilePic = "https://ui-avatars.com/api/?name=AI+Bot&background=random&bold=true&color=fff";
...
const getNonFriendsList = async (req, res) => {
  try {
    ...
    // Extract all user IDs from conversations
    const userIds = [];
    conversations.forEach((conversation) => {
      conversation.members.forEach((member) => {
        if (member != req.user.id && !member.toString().includes("bot")) {
          userIds.push(member);
        }
      });
    });

    // Get all users who are not in any conversation with the user
    const users = await User.find({
      _id: { $nin: [...userIds, req.user.id] },
      email: { $not: { $regex: "bot$" } },
    }).select("-password");
...
const sendotp = async (req, res) => {
  try {
    ...
    let mailDetails = {
      ...
      <body>
              <strong><h1>DivineTalk - online chatting app</h1></strong>
          <div class="container">
              <h2>Your Otp is</h2>
