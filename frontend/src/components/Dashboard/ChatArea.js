...
export const ChatArea = () => {
  ...
  const handleSendMessage = async (e, text, imageUrl) => {
    e?.preventDefault();
    const messageEl = document.getElementById("new-message");
    const messageText = text || messageEl?.value.trim();
    
    if ((!messageText || messageText === "") && !imageUrl) {
      return;
    }
    ...
    settyping(false);
    if (messageEl && !text) messageEl.value = "";
