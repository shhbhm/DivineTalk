...
const MyChatList = (props) => {
  ...
  useEffect(() => {
    socket.on("new-message-notification", async (data) => {
      ...
      var newlist = [...chatlist]; // Create a new array to avoid mutating state directly

      let chatIndex = newlist.findIndex(
        (chat) => chat && chat._id === data.conversationId
      );

      // If chat doesn't exist and we have conversation data, add it
      if (chatIndex === -1 && data.conversation) {
        newlist.unshift(data.conversation);
        chatIndex = 0;
      }

      // Only proceed if we found the chat
      if (chatIndex !== -1) {
        ...
        // Find the sender safely
        let sender = newlist[0]?.members?.[0];

        // Only play sound and show toast if not in the active chat
        if (activeChatId !== data.conversationId) {
